...
tags: 
  ...
  - "ruby"
  - "ruby-on-rails"
coverImage: "AdobeStock_298586120-scaled-1.jpeg"
---
...
I initially tried to save the whole object in the session, but the object turned out to be too large to comfortably fit in the session. We can, however, turn the LaunchContext into an ActiveRecord, so it can be saved in the database (we could also stick it in a memory cache, like Redis, if available).
...
db/migrate/...\_create\_launch\_contexts.rb
...
3. Because we'll be attaching some logic to our response form, we extract it to a ViewComponent to better handle these capabilities.

5. Give our response form component the ability to render the output HTML and encode it in a JWT, which is what we send back to the LMS.

7. Set up our controller to instantiate the required objects and pass them to the view.

9. Render our component.

We give ourselves access to the `LaunchContext` that we've saved in the session.

app/controllers/tools\_controller.rb
...
app/component/deep\_link\_response\_form\_component.rb
...
app/components/deep\_link\_response\_form\_component.html.erb
...
app/models/lti/response\_context.rb
